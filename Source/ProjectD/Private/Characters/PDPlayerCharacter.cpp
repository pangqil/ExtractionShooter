...
bool APDPlayerCharacter::TryAutoEquipWeaponSlot(const FPDInventorySlot& ItemSlot)
{
	...
	// ???ÅÏÜç?îÎêú ?îÌÉÑ??PickupWeapon ?∏Ï∂ú ?ÑÏóê ?ÅÏö© ??PickupWeapon ?¥Î???SwitchToSlot??	//    OnWeaponSwappedÎ•?broadcast?òÍ∏∞ ?ÑÏóê actor??CurrentAmmoÍ∞Ä ?ïÌôï??Í∞íÏù¥?¥Ïïº
	//    UIÍ∞Ä Ï≤??úÏãúÎ∂Ä???¨Î∞îÎ•??îÌÉÑ??Î≥¥ÏûÑ.
                                                                  ...
                                                                  bool bDestroyWeaponActor)
{
	...
	// ??destroy ?¥Ï†Ñ???∞Ì????ÅÌÉú Ï∂îÏ∂ú ???∏Ï∂ú?êÍ? ?∏Î≤§?†Î¶¨ ?¨Î°Ø??stamp ??
	...
	// Î©îÏù∏ Î¨¥Í∏∞Í∞Ä ÎπÑÏõåÏß??¨Ïã§??UI/Anim???åÎ¶º. ?§Ïôë ?êÎ¶Ñ???ºÎ??ºÎ©¥ ApplyÍ∞Ä Í≥???Î¨¥Í∏∞Î•??§Ïãú broadcast??
	if (bWasCurrent)
	{
		OnWeaponSwapped.Broadcast(nullptr, EWeaponSlot::None);
	}
