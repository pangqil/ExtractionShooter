...
APDEnemyBase::APDEnemyBase()
{
	...
	// ??AI ??Minimal ë¡?ì¶©ë¶„. (???°ì´?°ëŠ” ?¸ìŠ¤?¸ë§Œ ë³´ìœ )
	if (ASC)
	{
	...
	// Hostile ê¸°ë³¸ê°? ?ì‹(?? PDSoldier)?ì„œ ?¬ì •??ê°€??
	TeamID = 2;

	// ë¨¸ë¦¬ ??HUD ??Screen ê³µê°„ ?„ì ¯. ?”ë“œ ì¢Œí‘œë¥??”ë©´???¬ì˜??2D ?¤ë²„?ˆì´ë¡?ê·¸ë¦¬ë¯€ë¡?World ?€ë¹?ë¹„ìš©????Œ(ë¨¸í„°ë¦¬ì–¼Â·?Œë”?€ê¹ƒÂ·ë??ì²˜ë¦¬ ?†ìŒ).
	// ì£¼ì˜: ?”ë©´ê³µê°„?´ë¼ ??ƒ ì¹´ë©”?¼ë? ?¥í•˜ê³??¤í´ë£¨ì „???ìš©?˜ì? ?Šìœ¼ë©? ë¡œì»¬ PlayerController ê¸°ì??¼ë¡œë§??Œë”?œë‹¤.
...
void APDEnemyBase::BeginPlay()
{
	...
	// ?¬ìˆ˜ë³?ê°œì¸ ì¡°ì? ?¸í–¥ ???¸ìŠ¤?´ìŠ¤ë§ˆë‹¤ ??ë²?êµ´ë ¤ ê²Œì„ ?´ë‚´ ê³ ì •.
	// Pitch ??Offset(ë¬´ê¸° forward?’ëª¸??ë³´ì •) ì¤‘ì‹¬??Â±Max ?°í¬. Yaw ??0 ì¤‘ì‹¬??Â±Max ?°í¬.
	...
	// ?¤ë²„?¤ë“œ HUD ì´ˆê¸°?????„ì ¯ ?´ë˜??ë¯¸ì?????ìºì‹œ??nullptr ë¡??¨ì•„ ëª¨ë“  Show* ?¸ì¶œ??no-op.
	...
	// Torso HP ë³€ê²?êµ¬ë… + ì´ˆê¸°ê°?push. ShowHealth() ?„ê¹Œì§€ ?„ì ¯?€ collapsed ??ì²?push ??ë°±ê·¸?¼ìš´??ê°±ì‹ .
	if (ASC && AttributeSet)
	{
...
void APDEnemyBase::TickFootstep(float DeltaSeconds)
{
	...
	{
		// ?•ì? ì¤‘ì—” ?„ì ê°?ë¦¬ì…‹ ???¤ìŒ ì¶œë°œ ì§í›„ ì¦‰ì‹œ ë°œìêµ???£¼ ë°©ì?.
...
void APDEnemyBase::OnTorsoHPChanged(const FOnAttributeChangeData& Data)
{
	...
	// ì²??¼í•´ ??HP ë°??êµ¬ ?¸ì¶œ. ?´íŠ¸ë¦¬ë·°??ë³µì œë¡??œë²„/?´ë¼ ?‘ìª½?ì„œ ë°œí™” ???´ë¼ ?„ì ¯??ì¼œì§.
...
{
	// ë¹?Biped ?”í´?? Biped ?ì‹ ?´ë˜?¤ê? AttributeSet ê¸°ë°˜?¼ë¡œ ?¤ì œ ?íƒœ ë°˜í™˜.
...
void APDEnemyBase::SetEnemyState(EPDEnemyState NewState)
{
	...
	// ê²Œì„?Œë ˆ?????œë²„ ê¶Œìœ„): ë£¨íŠ¸ ?œëÂ·ì½œë¦¬?„Â·í??´ë¨¸ ?•ë¦¬ ?? ?ê²© ?´ë¼?ì„  ?¤í–‰?˜ì? ?ŠìŒ.
	switch (NewState)
	{
	...
	// ?°ì¶œ?€ ?œë²„(ë¦¬ìŠ¨ ?¸ìŠ¤???¬í•¨)?ì„œ ì¦‰ì‹œ, ?ê²© ?´ë¼??CurrentState ë³µì œ ??OnRep_EnemyState ?ì„œ.
...
void APDEnemyBase::ApplyEnemyStateCosmetics(EPDEnemyState NewState)
{
	...
	// Dead ???„ì ¯ ?„ì²´ ?¨ê?(?œì²´ ??HP ?”ì¡´ ë°©ì?). ê·??¸ì—” ?íƒœ ë§í’??ê°±ì‹ .
	if (OverheadWidget)
	{
	...
	// ?¬ë§ ëª½í?ì£¼Â·ì‚¬?´ë“œ: ?ë™ ë³µì œ?˜ì? ?Šìœ¼ë¯€ë¡??íƒœ ë³µì œ ?œì ??ê°?ë¨¸ì‹ ??ì§ì ‘ ?¬ìƒ.
	if (NewState == EPDEnemyState::Dead)
	{
		if (DeathMontage)
		{
...
void APDEnemyBase::ApplyDamage_Implementation(const FPDDamageInfo& DamageInfo)
{
	...
	// ?¼ê²©?? ?´ì•„?ˆì„ ?Œë§Œ ?¬ìƒ. ?¬ë§ ?°ë?ì§€??OnEnterState_Dead ??DeathSound ë¡??¼ì›??
	...
	// Health bar visibility is handled by OnTorsoHPChanged so replicated clients also show it.
	// Combat/Dead??BTê°€ ?´ë? ?¥ë™ ?€??ì¤‘ì´ê±°ë‚˜ ì¢…ë£Œ???íƒœ ??ì¶”ì  ? í˜¸ ë¬´ì‹œ.
	...
	// Controllerê°€ Instigatorë¡??¤ì–´??ê²½ìš° Pawn?¼ë¡œ ë³´ì • (?„ì¹˜ ? ë¢°???•ë³´).
	...
	// ê°™ì? ?€??ê°€???°ë?ì§€??ì¶”ì  ?¸ë¦¬ê±?ë¬´ì‹œ (?¤ë°œ/AOE ??.
	...
	// BB ì§ì ‘ ê°±ì‹  ??AIController??OnNoiseHintChangedë¥?êµ¬ë…?˜ì? ?Šìœ¼ë¯€ë¡?HandleNoiseHeard?€ ?™ì¼ ?¨í„´?¼ë¡œ ì±„ìš´??
...
{
	// ì¶©ëŒ/?´ë™ ?•ë¦¬. ?œì²´ê°€ ë°œì‚¬ì²´ë? ë§‰ì? ?Šë„ë¡? (StimuliSource ?´ì œ??ë² ì´??HandleDeath ?ì„œ.)
	...
	// ?¬ë§ ???œë ?ë¦„ ???œì„œ ì¤‘ìš”:
	//  1) ?œì²´ ì»¨í…Œ?´ë„ˆ(LootBox) ë¨¼ì? ?¤í° ???„ì† ?¨ê³„ê°€ LootBox ??LootComponent ??ì§ì ‘ ?ì¬.
	//  2) LootTable ?„ì´??ì¶”ê? (LootBox ?°ì„ , ì»¨í…Œ?´ë„ˆ ?†ìœ¼ë©??”ë“œ ?¡í„°ë¡??´ë°±).
	//  3) ?¥ì°© ë¬´ê¸° ?•ë¥  êµ´ë ¤ LootBox ??ì¶”ê?.
	SpawnCorpseContainer();
	DropLootOnDeath();
	TryDropEquippedWeaponToCorpse();

	// Death montage and sound are played by ApplyEnemyStateCosmetics on server and clients.
	// 0 ?´í•˜ë©??êµ¬ ë³´ì¡´(?œì²´ ì»¨í…Œ?´ë„ˆë¡??€ì²´ë˜???œë‚˜ë¦¬ì˜¤ ?? ??ê·??¸ì—??LifeSpan ?ˆì•½.
...
void APDEnemyBase::HandleDeath(AActor* Killer)
{
	...
	// AI ?˜ì‚¬ê²°ì • ì¦‰ì‹œ ?•ì? ??ì»¨íŠ¸ë¡¤ëŸ¬??ë³´ì¡´(?¬ë§ ëª½í?ì£??„ì† ì²˜ë¦¬ ?ˆì „), ?¡í„° ?Œë©¸ ??OnUnPossess ê°€ ?¼ê´„ ?•ë¦¬.
...
void APDEnemyBase::DropLootOnDeath()
{
	...
	// LootBox ê°€ ?œì²´ ì»¨í…Œ?´ë„ˆë¡??¤í°???ˆìœ¼ë©?LootBox ??LootComponent ë¡?ì§ì ‘ ?ì¬.
	// ì»¨í…Œ?´ë„ˆê°€ LootBox ê°€ ?„ë‹ˆê±°ë‚˜ ?†ìœ¼ë©??”ë“œ ?¡í„°ë¡??´ë°±.
	...
	for (const FPDLootEntry& Entry : LootTable)
	{
		...
		// LootBox ?°ì„  ??ItemID ë§??ˆìœ¼ë©?ì»´í¬?ŒíŠ¸ê°€ DT ì¡°íšŒ?´ì„œ ì²˜ë¦¬.
			...
			if (!bAdded)
			{
				UE_LOG(LogTemp, Warning,
					TEXT("[%s] DropLootOnDeath: LootComponent->AddItemByID failed for ItemID=%s. "
					     "LootComponent ??ItemDataTable ???¤ì •???ˆê³  ?´ë‹¹ row ê°€ ì¡´ì¬?˜ëŠ”ì§€ ?•ì¸."),
					*GetName(), *Entry.ItemID.ToString());
			}
			continue;
		}

		// ?´ë°±: ?”ë“œ ?¡í„° ?¤í° (LootBox ì»¨í…Œ?´ë„ˆ ?†ê±°??ItemID ë¯¸ì„¤????.
...
void APDEnemyBase::TryDropEquippedWeaponToCorpse()
{
	...
	// ?•ë¥  êµ´ë¦¼ ???¤íŒ¨ ??ë¬´ê¸°???œì²´ ?¡í„° ?Œë©¸ ??ê°™ì´ ?¬ë¼ì§?(?ì‹ OnEnterState_Dead ?ì„œ Destroy).
	...
	if (!bAdded)
	{
		UE_LOG(LogTemp, Warning,
			TEXT("[%s] Weapon drop AddItemByID failed ??ItemID=%s. "
			     "LootComponent ??ItemDataTable ë¯¸ì„¤???ëŠ” DT row ë¶€??"),
