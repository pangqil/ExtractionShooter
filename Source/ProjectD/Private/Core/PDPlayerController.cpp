                                                                       ...
                                                                       float RaidDurationSeconds)
{
	...
	{
		UE_LOG(LogPDCharacter, Warning, TEXT("Client_ShowRaidEndTransition: RaidEndTransitionClass ÎØ∏Ìï†??(PC=%s)"), *GetName());
		return;
	}
	...
	if (!Transition)
	{
		UE_LOG(LogPDCharacter, Warning, TEXT("Client_ShowRaidEndTransition: PushToLayer ?§Ìå® ?êÎäî Ï∫êÏä§???§Ìå® (PC=%s)"), *GetName());
		return;
	}
...
void APDPlayerController::Client_ShowRaidStartTransition_Implementation(const FText& ZoneName)
{
	...
	{
		UE_LOG(LogPDCharacter, Warning, TEXT("Client_ShowRaidStartTransition: RaidStartTransitionClass ÎØ∏Ìï†Îãπ (PC=%s)"), *GetName());
		return;
	}
	...
	if (!Transition)
	{
		UE_LOG(LogPDCharacter, Warning, TEXT("Client_ShowRaidStartTransition: PushToLayer Ïã§Ìå® ÎòêÎäî Ï∫êÏä§Ìä∏ Ïã§Ìå® (PC=%s)"), *GetName());
		return;
	}
...
void APDPlayerController::StartSpectatingDeath(APlayerController* InitialTarget)
{
	...
	// Listen-server host(?úÎ≤Ñ=Î°úÏª¨)?êÏÑú??OnRep ???êÎèô Î∞úÌôî?òÏ? ?äÏúºÎØÄÎ°??òÎèô Î∏åÎ°ú?úÏ∫ê?§Ìä∏.
...
void APDPlayerController::CycleSpectateTargetServer(int32 Direction)
{
	...
	if (Candidates.Num() == 0)
	{
		// ???¥ÏÉÅ ?¥ÏïÑ?àÎäî Í¥Ä???Ä?ÅÏù¥ ?ÜÏùå. ?êÍ∏∞ ?êÏã† ?úÏ†ê?ºÎ°ú ?§ÎÉÖ?òÎ©¥ Ï£ΩÏ? ?êÍ∏∞ Î™∏ÏúºÎ°?Ïª∑Ïù¥ ?Ä??		// Î∂Ä?êÏó∞?§Îü¨?∞Î?Î°?ÎßàÏ?ÎßâÏúºÎ°?Î≥¥Îçò ??Î∞©Í∏à Ï£ΩÍ±∞??Ï∂îÏ∂ú???Ä?????úÏ†ê??Í∑∏Î?Î°??†Ï?.
		// EndRaid Í∞Ä Í≥?Í≤∞ÏÇ∞ ?ÑÏ†Ø???ÑÏö∞ÎØÄÎ°?Ïπ¥Î©î???ÅÌÉú??Í±∞Í∏∞????ûÑ.
...
void APDPlayerController::SetupInputComponent()
{
	...
	// Step 2-B: Í¥Ä???§Ïùå/?¥Ï†Ñ (?¨Îßù ?ÑÏóêÎß??úÏÑ±?????∏Îì§?¨Ïóê??bIsSpectating Ï≤¥ÌÅ¨).
...
void APDPlayerController::BeginPlay()
{
	...
	// Î©Ä?? ?ÖÎ†•Î™®Îìú/HUD/RootLayout/Subsystem ?±Î°ù?Ä Î°úÏª¨ PC?êÏÑúÎß??òÎ?Í∞Ä ?àÏùå.
	// ?úÎ≤Ñ???àÎäî ?êÍ≤© ?¥Îùº?¥Ïñ∏?∏Ïùò PC ?∏Ïä§?¥Ïä§Í∞Ä ?¨Í∏∞ ?§Ïñ¥?§Î©¥ GI ?®Ïùº RootLayout??ÎßàÏ?Îß??∏Ï∂ú????ñ¥?∞Îäî ?¨Í≥†Í∞Ä ??
	if (!IsLocalController())
	{
		return;
	}
...
void APDPlayerController::PreClientTravel(const FString& PendingURL, ETravelType TravelType, bool bIsSeamlessTravel)
{
	...
	// ?¥Îùº?¥Ïñ∏??Î®∏Ïã†??PCÎß???hook?ºÎ°ú LoadingScreen???ÑÏ?.
	// ?∏Ïä§??=Authority)???êÍ∏∞ ?∏ÎûòÎ∏???TravelToLevel?êÏÑú ?¥Î? ShowImmediate ?∏Ï∂ú?òÎ?Î°??úÏô∏ ??	// Í∑∏Î†áÏßÄ ?äÏúºÎ©????¥Îùº?¥Ïñ∏??joinÏ≤òÎüº ?∏Ïä§??Ï∏°Ïóê PreClientTravel???¥Îñ§ Í≤ΩÎ°úÎ°úÎì† trigger????Ï§ëÎ≥µ ?úÏãú??
...
void APDPlayerController::OnPossess(APawn* InPawn)
{
	...
	// Step 2-B ?ÑÏÜç: ????ÎπôÏùò ??Í¥Ä???ÅÌÉú carry-over Ï∞®Îã®. HUD "Spectating: ..." ?îÏ°¥ Î∞©Ï?.
...
// codex Î∏åÎûúÏπòÎäî InventoryWidget ?ºÏù¥?ÑÏÇ¨?¥ÌÅ¥??UIManager Î°??ÑÏûÑ?òÎ?Î°?develop Íµ¨ÌòÑ???®Ïàú????// LootWidget Îß?PlayerController Í∞Ä ÏßÅÏ†ë ?§Î£®Í≥? ?ÖÎ†• Ï∞®Îã®?Ä Í∏∞Ï°¥ ?¨Ìçº ?¨ÏÇ¨??
void APDPlayerController::OpenLootInterface(UPDLootComponent* LootSource)
{
	...
	// ?§Î•∏ Î™®Îã¨ UI Í∞Ä Í∞ôÏ? viewport ?ÅÏó≠???êÏú†?òÎ?Î°?Î®ºÏ? ?´Ïùå.
	...
	// Stash/Market Ï≤òÎüº InventoryWidget ??Í∞ôÏù¥ ?ÑÏ? ??Loot ?Ä ?∏Î≤§ Í∞??úÎûòÍ∑?ÎπÑÍµê Í∞Ä??
	if (UIManagerComponent)
	{
...
void APDPlayerController::CloseLootInterface()
{
	...
	// Loot ?ÑÏ†ØÍ≥??®Íªò ?ÑÏõ†??InventoryWidget ???¥Ï†ú (?§Î•∏ UI Í∞Ä ?†Ï? ??????.
	if (UIManagerComponent)
	{
