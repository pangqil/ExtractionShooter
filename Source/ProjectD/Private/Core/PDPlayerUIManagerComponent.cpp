...
void UPDPlayerUIManagerComponent::ApplyEffectiveUIState(EWidgetInputMode Mode)
{
	...
	// Stack??effective top ëª¨ë?????°ë?? PC InputMode + ì»¤ì??ë¥??¼ê? ????.
	// stack??ë¹?ë©? Subsystem??Game??broadcast ???¬ê¸°??GameOnlyë¡?ë³µì?? ??WASD/Tab ??PC ???? ??????
	switch (Mode)
	{
...
{
	// Loot ???? RenderTransform ë¨¼ì? ë¦¬ì?? ???¤ë¥¸ UI ê°? ?¸ë²¤??ë¦? ??????ë©? ê·¸ë?ë¡?ë³´ì?´ë??ë¡?????ì¹?????.
	if (InventoryWidgetInstance)
	{
		InventoryWidgetInstance->SetLootCompanionMode(false);
	}

	// ?¤ë¥¸ UI(Stash/Market/Equip) ê°? ?¸ë²¤??ë¦? ???? ????ë©? ??ê±?.
...
void UPDPlayerUIManagerComponent::ToggleHub()
{
	...
	// PSê°? ?´ì?? ë§???ì°????? ?????¼ë??ë¡?ë§?????ë§??? ë©±ë?? ë°?????ë³´ê?.
	EnsureRaidParticipationBinding();

	// ???´ë??ì»¨í???¤í??(Stash/Market/Equip)ê°? ?´ë?????¼ë??ê·¸ê?ë¶????«ë????2?¨ê? close UX).
	...
	// ?¬ë?/Downed/ê´???ì¤???????ë¡? ?´ê¸° ê±°ë?. ?«ê¸°?????????´ë? ì²?ë¦???
	if (!CanOpenHub())
	{
		return;
	}

	// ë§???ë§????? ???¼ë??ë¹???ê·? ???? ??baseê°? DA??DefaultTabIdë¡?fallback.
...
{
	// Reset(????false) ???????? ?«ì?? ???? ????. ?¬ë?/ì¶?ì¶? ???? ????ë§???ë¦?.
	if (!bIsExtracted && !bIsRaidDead)
	{
		return;
	}

	// ???´ë??ì»¨í???¤í????base ?????´ë?? raid ì¤??? ?´ë?????? ???? ê°??¥ì?????¬ì?ë§???????ê²? ëª¨ë?? ??ë¦?.
